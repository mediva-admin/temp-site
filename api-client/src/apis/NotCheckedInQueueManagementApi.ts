...
export interface NotCheckedInQueueManagementApiInterface {
    /**
     * Check in a patient from reception queue to the first terminal queue of the flow. This endpoint takes only the patientId and handles the entire check-in process internally.  **Process Flow:** 1. **Validate Patient Flow**: First checks if the patient has a valid flow with at least 1 flow entry 2. **Save Patient Flow**: Saves the patient flow to ensure data consistency 3. **Queue Transfer**: Moves the patient from the not-checked-in queue to the first terminal queue 4. **Position Calculation**: Calculates the appropriate position in the terminal queue  The patient is moved from the not-checked-in queue to the first terminal queue based on their flow configuration. Hospital ID is automatically obtained from the request context. 
     * @summary Check in patient from reception queue to terminal queue
     * @param {string} patientId Unique identifier for the patient
     * @param {*} [options] Override http request option.
     * @throws {RequiredError}
     * @memberof NotCheckedInQueueManagementApiInterface
     */
    checkInPatientRaw(requestParameters: CheckInPatientRequest, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<runtime.ApiResponse<CheckInResponse>>;

    /**
     * Check in a patient from reception queue to the first terminal queue of the flow. This endpoint takes only the patientId and handles the entire check-in process internally.  **Process Flow:** 1. **Validate Patient Flow**: First checks if the patient has a valid flow with at least 1 flow entry 2. **Save Patient Flow**: Saves the patient flow to ensure data consistency 3. **Queue Transfer**: Moves the patient from the not-checked-in queue to the first terminal queue 4. **Position Calculation**: Calculates the appropriate position in the terminal queue  The patient is moved from the not-checked-in queue to the first terminal queue based on their flow configuration. Hospital ID is automatically obtained from the request context. 
     * Check in patient from reception queue to terminal queue
     */
    checkInPatient(patientId: string, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<CheckInResponse>;
...
export class NotCheckedInQueueManagementApi extends runtime.BaseAPI implements NotCheckedInQueueManagementApiInterface {
     ...
     * Check in patient from reception queue to terminal queue
     */
    async checkInPatientRaw(requestParameters: CheckInPatientRequest, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<runtime.ApiResponse<CheckInResponse>> {
        if (requestParameters['patientId'] == null) {
            throw new runtime.RequiredError(
                'patientId',
                'Required parameter "patientId" was null or undefined when calling checkInPatient().'
            );
        }
        ...
        let urlPath = `/api/not-checked-in/check-in/{patientId}`;
     ...
     * Check in patient from reception queue to terminal queue
     */
    async checkInPatient(patientId: string, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<CheckInResponse> {
        const response = await this.checkInPatientRaw({ patientId: patientId }, initOverrides);
